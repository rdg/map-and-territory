...
function main() {
  ...
  if (branch === "main") {
    console.error(
      `${RED}Blocked: committing directly to 'main' is disabled. Create a branch and PR.${RESET}`,
    );
    process.exit(1);
  }
  ...
  if (crlf.length > 0) {
    ...
    console.error(
      "Please convert to LF line endings (e.g., 'git config core.autocrlf input'), reformat, and re-stage.",
    );
    process.exit(1);
  }
