    ...
    <div className={cn('flex flex-col gap-1', className)}>
      ...
      <div className="flex items-center gap-2">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="grid grid-cols-7 gap-1 p-2 min-w-[0]">
            {presets.map((c) => (
              <DropdownMenuItem key={c} onClick={() => onChange?.(c)} className="p-0">
                <div className="h-6 w-6 rounded border" style={{ backgroundColor: c }} aria-label={c} />
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
